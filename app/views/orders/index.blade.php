...
@if (Route::current()->getUri()=='orders/list')
...
<div class="form-group form-group-sm col-xs-2 col-md-2">
 {{ Form::label('start_date', Lang::get('gui.timeframe'), array('class'=>'control-label')) }}
        ...
        <div class="form-group form-group-sm col-xs-2 col-md-2">
         {{ Form::label('start_date', '&nbsp;', array('class'=>'control-label')) }}
...
<table class="table table-condensed table-hover table-striped" id="grid-data-api">
	<thead>
		<tr>
			<th data-column-id="order_num" data-sortable="false" data-header-css-class="colbutton" data-formatter="order">{{
				Lang::get('gui.order') }}</th>
				<th data-column-id="order_date" data-header-css-class="colbutton">{{ Lang::get('gui.date')
				}}</th>

			<th data-column-id="order_type" data-header-css-class="type">{{ Lang::get('gui.type')
				}}</th>
			<th data-column-id="transporter_cname" data-header-css-class="time">{{
				Lang::get('validation.attributes.transporter_fk') }}</th>
				<th data-column-id="buyer_cname" data-header-css-class="time">{{
				Lang::get('gui.buyer') }}</th>
				<th data-column-id="vehicle_type" data-header-css-class="colbutton">{{ Lang::get('gui.vehicle') }}</th>
	            ...
	            $('#sDate').data("DateTimePicker").maxDate(e.date);
				   ...
				   var grid=$("#grid-data-api").bootgrid({
	                    ...
	                    post: function ()
	                    {
	            			...
	            			url: '{{{ URL::action('OrdersController@orderList') }}}',
	            			formatters: {
