@extends('layouts.master') @section('title') @parent :: Companies @stop
...
function destroyBusiness(){
    ...
    if (row){
        ...
        $.messager.confirm('Pažnja!','Jeste li sigurni da želite obrisati ovaj podatak?',function(r){
            if (r){
                $.post('companies/'+row.id,{id:row.id, _method:'DELETE',_token:'<?=csrf_token(); ?>'},function(result){
                    if (result.success){
                        $('#business-datagrid').datagrid('reload');    // reload the user data
                        $.messager.show({    // show message
                            title: result.status,
...
$(function(){
	...
	$('#companies_type_fk').combobox({
		url: '{{{ URL::action('CompanyTypesController@index') }}}',
		method: 'GET',
		valueField:'id',
		textField:'type'
	});
	...
	$('#business-datagrid').datagrid({
		...
		@if (Route::current()->getUri()=='clients')  
			...
			@else (Route::current()->getUri()=='contacts')
