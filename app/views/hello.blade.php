<!--
<!doctype html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Laravel PHP Framework</title>
	<style>
		@import url(//fonts.googleapis.com/css?family=Lato:700);

		body {
			margin:0;
			font-family:'Lato', sans-serif;
			text-align:center;
			color: #999;
		}

		.welcome {
			width: 300px;
			height: 200px;
			position: absolute;
			left: 50%;
			top: 50%;
			margin-left: -150px;
			margin-top: -100px;
		}

		a, a:visited {
			text-decoration:none;
		}

		h1 {
			font-size: 32px;
			margin: 16px 0 0 0;
		}
	</style>
</head>
<body>
	<div class="welcome">
-->
@extends('layouts.master')

@section('title')
@parent
:: Home
@stop

@section('content')
		<a href="http://laravel.com" title="Laravel PHP Framework"><img src="data:image/png;base64," alt="Laravel PHP Framework"></a>
<h1>You have arrived.</h1>

{{ Form::open(array('url' => 'foo/bar','class'=>'form-horizontal form-group-sm','role'=>'form')) }}
<div class="form-group">
{{ Form::label('transporter_fk', 'Transporter', array('class'=>'col-sm-2
			control-label')) }}
<div class="col-sm-4">
{{ Form::select('transporter_fk',array(),1,array('id'=>'transporter_fk','class'=>'easyui-combobox','style'=>'width:200px')) }}
</div>
</div>
<div class="form-group">
{{ Form::label('vehicletype_fk', 'Tip Vozila', array('class'=>'col-sm-2
			control-label')) }}
			<div class="col-sm-4">
{{ Form::select('vehicletype_fk',array(),1,array('id'=>'vehicletype_fk','class'=>'easyui-combobox','style'=>'width:200px')) }}</div>
</div>
<div class="form-group">
{{ Form::label('vehicle_id', 'Vozila', array('class'=>'col-sm-2
			control-label')) }}
			<div class="col-sm-4">
{{ Form::select('vehicle_id',array('0'=>'Nema Rezultata'),1,array('id'=>'vehicle_id','class'=>'easyui-combobox','style'=>'width:200px')) }}</div>
</div>

{{ Form::close() }}

@stop

@section ('extrascripts')
<script>
$( document ).ready(function() {
// 	$.ajax({
// 	    url: '{{{ URL::action('TransportersController@listAjax') }}}',
// 	    type: "GET", 
// 	    success: function(data){
// 	      $('#transporter_fk').empty();
// 	      $('#transporter_fk').append(new Option('Odaberite Transportera', 0));
// 	        $.each(data, function() {
// 	            $('#transporter_fk').append(new Option(this.cname, this.id));
// 	        });
// 	        }
// 	  });

	$('#transporter_fk').combobox({
		url: '{{{ URL::action('TransportersController@listAjax') }}}',
		method: 'GET',
		valueField:'id',
		textField:'cname',
		
		onSelect: function(record) {
			
			$('#vehicle_id').combobox({
				url: "/vehicles/ajax/"+record.id+"/"+$('#vehicletype_fk').combobox('getValue'),
				method: 'GET',
				valueField:'id',
				textField:'licenceplate',
				panelHeight:'auto'
			});

		}
		
		});

	$('#vehicletype_fk').combobox({
		url: '{{{ URL::action('VehicleTypesController@listAjax') }}}',
		method: 'GET',
		valueField:'id',
		textField:'type',
		onSelect: function(record) {
			
			$('#vehicle_id').combobox({
				url: "/vehicles/ajax/"+$('#transporter_fk').combobox('getValue')+"/"+record.id,
				method: 'GET',
				valueField:'id',
				textField:'licenceplate',
				panelHeight:'auto'
			});

		}
		
		});

		  
	});





$('#transporter_fk').change(function(){
	$.ajax({
	    url: "http://localhost:8000/vehicles/ajax/"+$(this).val()+"/"+$('#vehicletype_fk').val(),
	    type: "GET", 
	    success: function(vehicles){
	      $('#vehicle_id').empty();
	        $.each(vehicles, function() {
	            $('#vehicle_id').append(new Option(this.licenceplate, this.id));
	        });		          
	      }
	  });
});

$('#vehicletype_fk').change(function(){
	$.ajax({
	    url: "http://localhost:8000/vehicles/ajax/"+$('#transporter_fk').val()+"/"+$(this).val(),
	    type: "GET", 
	    success: function(vehicles){
	      $('#vehicle_id').empty();
	        $.each(vehicles, function() {
	            $('#vehicle_id').append(new Option(this.licenceplate, this.id));
	        });		          
	      }
	  });
});


</script>



@stop
<!--
	</div>
</body>
</html>
-->
